...
<div id="sidebar" class="border-end d-flex flex-column pb-3 ${active}">
  <div class="d-flex">
    ...
    <a
      href="${home}"
      class="link-light d-flex py-3 mx-auto align-items-center justify-content-center link-underline link-underline-opacity-0"
    >
  ...
  <div
    id="sidebar_items"
    class="my-3 overflow-y-auto overflow-x-hidden"
    style="flex: 1"
  >
    <div class="list-group list-group-flush">
      ...
      <c:forEach items="${sideBarUI}" var="c">
        ...
        <c:choose>
          <c:when test="${isDebugging == 1}">
            <a
              href="${url}"
              type="button"
              class="d-flex align-items-center list-group-item list-group-item-action ${active}"
            >
              <i class="${c.icon}"></i>
              <span
                class="sidebar_li_item_text ms-3 d-inline-block text-nowrap"
              >
          ...
          <c:otherwise>
            <c:choose>
              ...
              <c:when test="${authenticated_user.roleId.id ==4}">
                <c:if
                  test="${c.code == 0 || c.code == 3 || c.code == 4 || c.code == 2 || c.code ==8}"
                >
                  <a
                    href="${url}"
                    type="button"
              ...
              <c:when test="${authenticated_user.roleId.id ==3}">
                <c:if
                  test="${ c.code == 0 || c.code == 3 || c.code == 4 || c.code == 7 || c.code == 2 || c.code == 8}"
                >
                  <a
                    href="${url}"
                    type="button"
