            ...
            <div class="input-group">
                ...
                <form:select class="form-select" path="tripId">
                    <c:forEach items="${trips}" var="c">
                        ...
                        <c:if test="${c.id == ticket.tripId.id}">
                            <c:set value="selected" var="selected" />
                        </c:if>
                        <option value="${c.id}" ${selected}>
                            ${c.routeId.startLocation} - ${c.routeId.endLocation} -
                            <fmt:formatDate value="${c.trip.startAt}" pattern="${date_pattern}" />
                        </option>
                    </c:forEach>
                </form:select>
            ...
            <div class="input-group mb-2">
                ...
                <form:select class="form-select" path="busId" id="busSelect">
                    <c:forEach items="${buses}" var="c">
                        <c:set value="" var="selected" />
                        <c:if test="${c.id == ticket.busId.id}">
                            <c:set value="selected" var="selected" />
            ...
            <div class="input-group">
                ...
                <form:select class="form-select" path="driverId">
                    <c:forEach items="${drivers}" var="c">
                        <c:set value="" var="selected" />
                        <c:if test="${c.id == ticket.driverId.id}">
                            <c:set value="selected" var="selected" />
                        </c:if>
